...
constexpr auto MAPLE_INJECT_DLLNAME = "TT.dll";			// name of dll to inject if above line is commented out
constexpr auto MAPLE_EXE_NAME = "TTC.dll";				// name of executable to start
constexpr auto MAPLE_STARTUP_ARGS = " 127.0.0.1 8484";	// maple exe arguments to use on startup (usually needs a space first)
...
constexpr auto IMGUI_INI_FILENAME = "TTG.dat";
...
#define MAPLE_INSTAJECT			TRUE	// if true dll will immediately edit memory after injecting (when target exe is unvirtualized)
...
#define MAPLETRACKING_GETPROCADDR		TRUE		// hook and log getprocaddr strings. logs all calls from the calling address after mutex is triggered, then it unhooks itself
